...
TEXTS = {
    ...
    "wrong_order": {
        ...
        "pl": "Naciśnij /start i postępuj zgodно з przyciskами 🙂",
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Язык ---
        ...
        # После выбора языка — сразу показываем меню темы
        themes = ["❤️ Love", "😄 Funny", "🎉 Celebration", "😢 Sad", "💍 Wedding", "🎼 Classic", "✍️ Custom", "🇵🇱 Disco Polo"]
        keyboard = [[InlineKeyboardButton(t, callback_data=f"theme_{i}")] for i, t in enumerate(themes)]
        await query.edit_message_text(
            t(uid, "choose_theme"),
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
...
def main():
    ...
    # Добавляем текстовый обработчик
