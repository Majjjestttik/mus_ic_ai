...
TEXT = {
    "start": {
        ...
        "fr": "🎵 *MusicAi*\n\nJe crée une chanson complète en quelques minutes.\nAppuie sur START 👇",
...
def t(uid, key):
    ...
    # Если ключ — это вложенный словарь (для языков)
    if isinstance(TEXT.get(key), dict):
        return TEXT[key].get(lang, TEXT[key].get("en", "Error: Text not found"))
    # Если ключ — это обычная строка
    return TEXT.get(key, "Error: Text not found")

# -------------------- /start --------------------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if uid not in users: users[uid] = {"lang": "en"}
    kb = [[InlineKeyboardButton("▶️ START", callback_data="start")]]
    await update.message.reply_text(t(uid, "start"), reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
